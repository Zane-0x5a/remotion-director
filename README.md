...
Good equipment gets you a good design on paper. The harder problem is that a model can't be trusted to tell when the *render* actually lands — and the loop is built, step by step, around that one fact.

It starts with a thing we kept watching happen: a model that grades its own frame while holding its own design doc doesn't really see the frame. It reads it *through* the doc and talks itself into "yes, this delivers." Three separate times, on three different briefs, the model signed off — "I smoothed the banding" — and a second pair of eyes that had never read the doc saw the cheap concentric rings instantly. The flaw was always there; the author just couldn't see it.
