...
好装备能让你在纸面上得到一份好设计。更难的问题是:模型靠不住——它判断不了*渲染*到底有没有兑现。整个环,是一步一步围着这一个事实搭起来的。

起点是一件我们反复看见的事:模型一边攥着自己的设计文档、一边给自己的帧打分时,它其实没在看那帧。它是*透过文档*看,然后把自己说服成「是的,兑现了」。三次,在三个不同的 brief 上,模型签字放行——「banding 我抹平了」——而另一双从没读过那份文档的眼睛,一眼就看见了那圈廉价的同心环。瑕疵一直在那儿,只是作者自己看不见。
