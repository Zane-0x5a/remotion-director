...
这个项目的大部分功夫不在管线代码,而在那套**设计装备**——模型在抽出第一帧之前,当成自己常驻知识去读的那段文字。它被反复研究、调试、验证,直到能稳定地把模型从那个安全的、平均的答案上推开。挑三条它真正承重的规则,看每条到底是为了什么:

| 规则 | 它杀掉的 slop | 换来的东西 |
|---|---|---|
| **先钉死一个可证伪的 conceit**——全片靠它一条机制立住,而且主角必须是*一个事物或事件*(一把空椅子、一行字的命运),绝不是光效或氛围。 | 品类反射:一句「深夜图书馆」,换来的永远是暖渐变上的居中大字。 | 一支你能拿它自己的承诺去校验的片——上面那支宣发里,模型把自己的反射念出来(*「居中反射、暖渐变默认」*),再划掉。 |
| **按固定顺序设计**——先叙事,再质感,最后引擎计划——而且叙事先锁进文件,之后才允许读质感技法。 | 主角悄悄变成一个打光把戏:太早学会那些光效技法的模型,最后做出一支*关于光*的片。 | conceit 守住它真正的主角;质感为故事服务,而不是顶替故事。 |
| **以设计师身份自检,而不是质检员。** 全项目调得最狠的一句:模型一回看自己的渲染,就不设计了、开始打勾——「可接受残差,交吧」。 | 一份满是野心的初稿,在技术上刚能渲出来的那一刻就被签字放行。 | 一个会盯着真像素问「这配得上我要的那支片吗」的创作者——配不上,就回去改设计。 |

装备抬的是**地板**——它让一次大胆而称职的尝试变得可靠。天花板是另一个问题,那正是环要解决的。(完整七轴——叙事、美学、色彩、构图、节奏、说服、质感——以及「装备显著优于裸 brief」的实验,都在 [`docs/WHY.md`](docs/WHY.md)。)
